...
object NFC {
    private const val moneySector = 9
    private const val otherSector = 8
        ...
        try {
            ...
            if (auth) {
                if(allowedCards == null || allowedCards.contains(byteArrayToHexString(myTag!!.id))) {
                    if(amount==null){
                        mfc.writeBlock(moneySector, ByteArray(sectorSize))
                        mfc.writeBlock(otherSector, ByteArray(sectorSize))
                        return Pair(byteArrayToHexString(myTag!!.id), 0L)
                    } else {
                        val previousAmount = getNumberFromByteArray(mfc.readBlock(moneySector))
                        val newAmount: Long = maxOf(0, previousAmount + amount)

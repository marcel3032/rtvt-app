...
class AddMoneyFragment : Fragment(), NfcFragment {
    ...
    override fun doNfcIntent(intent: Intent) {
        ...
        var moneyToAdd: Long? = if(moneyView?.text.toString()=="") 0L  else moneyView?.text.toString().toLong()

        val resetView = view?.findViewById<CheckBox>(R.id.reset_game_money)
        if(resetView?.isChecked == true)
            moneyToAdd = null
